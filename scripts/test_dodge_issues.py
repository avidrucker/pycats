...
def test_dodge_issues():
    ...
    print(
        f"Before air dodge: pos={player1.rect.center}, vel={player1.vel}, on_ground={player1.on_ground}"
    )
    ...
    print(
        f"After air dodge trigger: state={player1.fsm.state}, vel={player1.vel}, spot_dodge_flag={player1.spot_dodge_shield_held}"
    )
    ...
    print(
        f"After side dodge trigger: state={player2.fsm.state}, vel={player2.vel}, spot_dodge_flag={player2.spot_dodge_shield_held}"
    )
    print(
        f"Image color after trigger: {player2.image.get_at((0, 0))}"
    )  # Check if stuck white
    ...
    for i in range(DODGE_TIME):
        ...
        if i == 0:
            ...
            print(
                f"Side dodge final frame: vel={player2.vel}, total_distance={total_distance}px"
            )
            print(
                f"Final image color: {player2.image.get_at((0, 0))}"
            )  # Should be normal color
