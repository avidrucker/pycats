...
def test_spot_dodge():
    ...
    player1 = Player(
        x=400,
        y=400,  # On the thin platform (y should match platform top)
        controls={
            "left": pg.K_a,
            "right": pg.K_d,
            "up": pg.K_w,
...
def test_player_spot_dodge(player, platforms, platform_type):
    ...
    print(
        f"After settling: Player position: {player.rect.center}, On ground: {player.on_ground}"
    )
    ...
    print(
        f"Frame 1 - Shield+Down pressed simultaneously. State: {player.fsm.state}, Spot dodge flag: {player.spot_dodge_shield_held}, Shield attempting: {player.shield_attempting}"
    )
    ...
    for i in range(3):
        ...
        if i == 0:
            print(
                f"Frame {i + 2} - Continuing to hold. State: {player.fsm.state}, Shield attempting: {player.shield_attempting}"
            )
    ...
    for i in range(DODGE_TIME):
        ...
        if i == 0:
            print(
                f"Dodge frame {i + 1} - State: {player.fsm.state}, Position: {player.rect.center}, On ground: {player.on_ground}"
            )
        elif i == DODGE_TIME - 1:
            print(
                f"Dodge frame {i + 1} (last) - State: {player.fsm.state}, Position: {player.rect.center}, On ground: {player.on_ground}"
            )
    ...
    print(
        f"After dodge - State: {player.fsm.state}, Position: {player.rect.center}, On ground: {player.on_ground}"
    )
    print(
        f"Shield attempting: {player.shield_attempting}, Spot dodge flag: {player.spot_dodge_shield_held}"
    )
    ...
    if player.fsm.state == "shield":
        print(
            f"✅ SUCCESS: Player transitioned to shield state after spot dodge on {platform_type}!"
        )
    else:
        print(
            f"❌ FAIL: Player state is '{player.fsm.state}' instead of 'shield' on {platform_type}!"
        )

    if not player.shield_attempting or player.fsm.state != "shield":
        print(
            f"✅ SUCCESS: No unwanted shield display during spot dodge on {platform_type}!"
        )
    else:
        print(
            f"⚠️  INFO: Shield state/attempting status on {platform_type}: state={player.fsm.state}, attempting={player.shield_attempting}"
        )
