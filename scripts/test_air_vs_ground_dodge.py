...
def test_air_vs_ground_dodge():
    ...
    print(
        f"Ground test - Initial: pos={player1.rect.center}, on_ground={player1.on_ground}"
    )
    ...
    print(
        f"Ground spot dodge - After trigger: state={player1.fsm.state}, spot_dodge_flag={player1.spot_dodge_shield_held}"
    )
    ...
    for i in range(5):
        ...
        if i == 0:
            print(
                f"Ground spot dodge - Frame {i + 1}: pos={player1.rect.center}, vel={player1.vel}, on_ground={player1.on_ground}"
            )
    ...
    print(
        f"Air test - Initial: pos={player2.rect.center}, on_ground={player2.on_ground}, vel={player2.vel}"
    )
    ...
    print(
        f"Air dodge - After trigger: state={player2.fsm.state}, spot_dodge_flag={player2.spot_dodge_shield_held}"
    )
    ...
    for i in range(5):
        ...
        if i == 0:
            print(
                f"Air dodge - Frame {i + 1}: pos={player2.rect.center}, vel={player2.vel}, on_ground={player2.on_ground}"
            )
        elif i == 4:
            print(
                f"Air dodge - Frame {i + 1}: pos={player2.rect.center}, vel={player2.vel}, on_ground={player2.on_ground}"
    ...
    # Results
    print("\n--- Results ---")
    if player1.spot_dodge_shield_held or (
        player1.fsm.state == "dodge" and abs(player1.vel.y) < 0.1
    ):
    ...
    if not player2.spot_dodge_shield_held and player2.vel.y > 0:
        ...
        print(
            f"   Debug: spot_dodge_flag={player2.spot_dodge_shield_held}, vel.y={player2.vel.y}"
        )
