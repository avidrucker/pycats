...
def main(argv=None, presenter=None):
    ...
    ap.add_argument(
        "--match",
        action="store_true",
        help="play a full match to defeat (P1 chase bot vs idle P2) instead of the fixed scripted replay",
    )
    ap.add_argument(
        "--vs",
        choices=["idle", "chase", "idler", "follower"],
        default="idle",
    ...
    ap.add_argument(
        "--uncapped",
        action="store_true",
        help="run the live window uncapped so the FPS readout shows the true achievable rate (default paces to 60)",
    )
    ap.add_argument(
        "--no-overlay", dest="overlay", action="store_false", help="hide the live FPS / stocks / damage overlay"
    )
    ap.add_argument(
        "--seed",
        type=int,
    ...
    ap.add_argument("--p2-char", choices=CHARACTERS, default=None, help="P2 character/archetype.")
    ap.add_argument(
        "--p1-level",
        type=int,
        choices=range(1, 10),
    ...
    if args.demo:
        ...
        controllers = (AttackerController(attacker_num=1, rng=rng), VS_CONTROLLERS[args.vs](attacker_num=2, rng=rng))
