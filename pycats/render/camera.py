...
(inner) is the camera's pan-limit — the camera clamps to it, so the stage stays
framed and the blast lines never show (this is what makes "stage always visible"
fall out for free, PM-faithful). **Dead** (outer) is the KO boundary (== the
on-branch wide-blast hack). Between them is the off-screen / magnifying-glass band.
The `--dev` overlay draws both boxes as outline-only rects on the background AND
relaxes the camera clamp from CamLimit to Dead, so all three zones are visible for
eyeballing. Feel constants (margin, lerp, zoom-in cap) are by-feel throwaway values;
the zoom-out ceiling is derived from the clamp box, not guessed.
"""
...
# +848 / +745 top / +486 bottom (Dead == Fighter._outside_blast_zone's wide-blast
# hack). Both derived from #790's CamLimit (±170, +114/-80) and Dead (±246, +188/-140)
