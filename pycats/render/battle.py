...
def render_battle(surface, players, platforms, offset=(0, 0)):
    ...
    `offset` (#1339 follow+zoom camera) shifts every world draw by (ox, oy) px so the
...
def render_world(surface, players, platforms, attacks, offset=(0, 0)):
    """The single offset-aware world-draw seam (#1339 D1): platforms + fighters +
    attacks — the pair every battle present path draws together (the live
    `battle_screen` and the sim/replay presenters). `offset` (0, 0) is identity /
    byte-identical; the follow+zoom camera (`render/camera.py`) passes its BASE_OFFSET
    so the world lands on its oversized surface through this same seam — one world-draw
    path, no camera-specific fork. HUD/overlay are the caller's, drawn in screen space
    after this so the camera transform never warps them."""
    render_battle(surface, players, platforms, offset=offset)
    render_attacks(surface, attacks, offset=offset)
