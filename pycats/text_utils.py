...
class TextRenderer:
    ...
    def _find_unicode_font(self):
        ...
        if default_result['score'] >= len(basic_chars):
            ...
            return {"name": "default", "supported_chars": set(default_result['supported'])}
        ...
        for font_name in regular_unicode_fonts:
            if font_name in available_fonts:
                ...
                if result['score'] == len(basic_chars):
                    ...
                    return {"name": font_name, "supported_chars": set(result['supported'])}
                else:
    ...
    def render_text_mixed(self, text, size, color, surface, position, center=False):
        ...
        if center:
            total_width = self._calculate_text_width(text, regular_font, unicode_font, supported_chars)
    ...
    def render_text_simple(self, text, size, color, surface, position, center=False, unicode_fallback=True):
        ...
        if center:
            ...
            return pygame.Rect(position[0], position[1], rendered_text.get_width(), rendered_text.get_height())
    ...
    def render_unicode_char(self, char, size, color, surface, position, center=False, fallback_char=None):
                ...
                if char in font_info["supported_chars"]:
                    try:
                        ...
                        if center:
                            ...
                            adjusted_position = (position[0], position[1] + baseline_adjustment)
                            surface.blit(char_surface, adjusted_position)
                            return pygame.Rect(adjusted_position[0], adjusted_position[1], 
                                             char_surface.get_width(), char_surface.get_height())
                    except Exception as e:
        ...
        if center:
            ...
            return pygame.Rect(position[0], position[1], char_surface.get_width(), char_surface.get_height())
