...
- **PM 3.6 specifically:** **[inference — now Brawl-baseline-pinned]** PM 3.6 is a Brawl mod and inherits Brawl's Controls-menu tap-jump toggle; SmashWiki's "Brawl onward" covers Brawl but does not name PM. Follow-up #874 could **not** locate a PM-3.6-specific primary (in-game controls screenshot / PM manual / PMDT changelog naming the toggle), but it **firmly pinned the Brawl baseline** the claim inherits from (verbatim SmashWiki *Controls* / *Tap jump* quotes) and strengthened the inheritance inference: PMDT's changelog records no controller-config change (⇒ retained, not modified) and community threads corroborate PM players disabling it. So this is **no longer a bare GUESS** — it rests on a pinned Brawl baseline plus a labeled inheritance inference; it firms to **FOUND** only on a PM-3.6-specific controls screenshot / manual. See `docs/research/2026-07-21-pm36-tapjump-toggle-primary.md` and `docs/pm-reference/input-model.md` (§2 tap-jump-toggle row).
