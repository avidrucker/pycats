...
def test_air_dodge_velocity():
    ...
    def test_air_dodge_direction(direction_name, direction_key, expected_vel_x):
        ...
        player = Player(400, 300, controls, (255, 160, 64), (255, 255, 255), f"{direction_name}AirCat")
        ...
        print(f"Before air dodge: pos={player.rect.center}, on_ground={player.on_ground}, vel={player.vel}")
        ...
        continue_frame = InputFrame(held={pg.K_q, direction_key}, pressed=set(), released=set())
        ...
        print(f"Distance moved in 5 frames: {distance}px (expected: {expected_distance}px)")
        ...
        if velocity_correct and movement_correct:
            ...
            if not movement_correct:
                print(f"   - Movement incorrect: got {distance}, expected {expected_distance}")
    ...
    controls = {'left': pg.K_a, 'right': pg.K_d, 'up': pg.K_w, 'down': pg.K_s, 'shield': pg.K_q, 'attack': pg.K_e}
    player_neutral = Player(400, 300, controls, (128, 128, 255), (255, 255, 255), "NeutralAirCat")
    ...
    print(f"After neutral air dodge: state={player_neutral.fsm.state}, vel={player_neutral.vel}")
