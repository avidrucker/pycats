...
def test_dodge_direction(direction_name, direction_key, dir_x):
    ...
    player = Player(400, 370, controls, (255, 160, 64), (255, 255, 255), f"{direction_name}Cat")
    ...
    print(f"After dodge trigger: state={player.fsm.state}, vel={player.vel}, dodge_timer={player.dodge_timer}")
    ...
    for frame in range(1, 15):  # DODGE_TIME is 14
        ...
        if frame <= 3 or frame >= 12:  # Show early and late frames
            print(f"Frame {frame}: x={current_x}, vel={player.vel}, state={player.fsm.state}, timer={player.dodge_timer}")
    ...
    print(f"Distance ratio: {total_distance / expected_distance if expected_distance != 0 else 'N/A'}")
    
    # Check if velocity was consistent
    dodge_velocities = [v for v in velocities[1:] if v != 0]  # Skip initial and zero velocities
    if dodge_velocities:
