...
def _make_player():
    ...
    p = Player(
        x=700, y=400, controls=CONTROLS, color=P1_COLOR, eye_color=WHITE, char_name="OrderCat", facing_right=True
    )
    p.update(InputFrame(held=set(), pressed=set(), released=set()), platforms, pg.sprite.Group())  # settle on ground
    return p, platforms
