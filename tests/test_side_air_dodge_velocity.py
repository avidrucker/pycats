...
def create_input_frame(
    shield_pressed=False, left_pressed=False, right_pressed=False, down_pressed=False
):
...
def test_left_air_dodge():
    ...
    print(
        f"After left air dodge: pos=({player.rect.centerx}, {player.rect.centery}), vel=({player.vel.x}, {player.vel.y})"
    )
    ...
    if abs(player.vel.y - initial_y_vel) > 1.0:  # Allow some tolerance for gravity
        print(
            f"❌ Expected Y velocity preserved at ~{initial_y_vel}, got {player.vel.y}"
        )
        success = False
...
def test_right_air_dodge():
    ...
    print(
        f"After right air dodge: pos=({player.rect.centerx}, {player.rect.centery}), vel=({player.vel.x}, {player.vel.y})"
    )
    print(f"State: {player.fsm.state}")
    print(f"Dodge timer: {player.dodge_timer}")
    print(f"Air dodge ok: {player.air_dodge_ok}")
...
def test_shield_then_direction_air_dodge():
    ...
    if abs(player.vel.x) < 0.1:
        print(
            f"❌ Expected horizontal velocity after direction press, got X vel = {player.vel.x}"
        )
        success = False
    else:
...
def test_air_dodge_vs_ground_dodge():
    ...
    print(
        f"Air dodge result: vel=({air_player.vel.x}, {air_player.vel.y}), spot_dodge={air_player.spot_dodge_shield_held}"
    )
    ...
    print(
        f"Ground dodge result: vel=({ground_player.vel.x}, {ground_player.vel.y}), spot_dodge={ground_player.spot_dodge_shield_held}"
    )
    ...
    if abs(air_player.vel.y - 3.0) > 1.0:  # Allow tolerance for gravity
        print(
            f"❌ Air dodge should preserve Y velocity, expected ~3.0, got {air_player.vel.y}"
        )
        success = False
    ...
    if abs(abs(air_player.vel.x) - abs(ground_player.vel.x)) > 0.1:
        print(
            f"❌ Air and ground dodge should have same horizontal speed, air={abs(air_player.vel.x)}, ground={abs(ground_player.vel.x)}"
        )
        success = False
