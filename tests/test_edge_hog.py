...
from pycats.entities.ledge import LEFT, ledge_intangible_frames, ledges_from_platforms
...
def test_ledge_intangible_frames_is_fixed_constant():
    ...
    assert ledge_intangible_frames() == config.LEDGE_INTANGIBLE_BASE_FRAMES
    assert config.LEDGE_INTANGIBLE_BASE_FRAMES == 21


def test_percent_does_not_change_ledge_intangible_window():
    ...
    assert high.fighter.ledge_intangible_timer == low.fighter.ledge_intangible_timer
    assert low.fighter.ledge_intangible_timer == ledge_intangible_frames()


def test_grab_sets_intangible_burst_not_full_hang():
    plats = _stage()
    ...
    assert p.fighter.ledge_intangible_timer == ledge_intangible_frames()
    assert p.fighter.ledge_intangible_timer == config.LEDGE_INTANGIBLE_BASE_FRAMES  # the fixed burst


def test_intangible_lapses_while_still_hanging():
    plats = _stage()
    ledges = ledges_from_platforms(plats)
    p = _player()
    p.fighter.percent = 0
    ...
    for _ in range(ledge_intangible_frames() + 2):
        p.update(_empty_frame(), plats, p_attack_group(), ledges)
        if p.state != "ledge_hang":
            break
    assert p.state == "ledge_hang"  # still hanging
    assert p.fighter.ledge_intangible_timer == 0
    assert p.fighter.intangible is False  # but no longer intangible
...
def test_hog_grab_succeeds_and_evicts_once_intangible_lapses():
    plats = _stage()
    ledges = ledges_from_platforms(plats)
    left = next(l for l in ledges if l.side == LEFT)
    p1 = _player()
    ...
    for _ in range(ledge_intangible_frames() + 1):
