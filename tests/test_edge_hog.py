...
from pycats.entities.ledge import Ledge, LEFT, ledges_from_platforms, ledge_invuln_frames
...
def test_higher_percent_grants_longer_ledge_invuln_window():
    ...
    low = _player(); low.fighter.percent = 0
    high = _player(); high.fighter.percent = 150
...
def test_hog_denies_grab_while_occupant_invincible():
    plats = _stage(); ledges = ledges_from_platforms(plats)
    p1 = _player(); p1.fighter.percent = 100   # long invuln burst
    p2 = _player(); p2.fighter.percent = 0
...
def test_hog_grab_succeeds_and_evicts_once_invuln_lapses():
    plats = _stage(); ledges = ledges_from_platforms(plats)
    left = next(l for l in ledges if l.side == LEFT)
    p1 = _player(); p1.fighter.percent = 0
    p2 = _player(); p2.fighter.percent = 0
    _grab_left(p1, plats, ledges)
