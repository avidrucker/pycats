"""Ledge-grab INVULN bar renders during the hang (#658, revives #531).

#531 added a green INVULN timer bar for `ledge_invuln_timer` but gated it on
...
stack. Spec: docs/pm-reference/ledge-regrab-invuln-and-display.md.
"""
...
def test_invuln_bar_renders_during_a_real_hang():
    ...
    assert p.fighter.ledge_invuln_timer > 0  # burst live
    specs = rb.timer_bar_specs(p)
    invuln = [b for b in specs if b.label == "INVULN"]
    assert len(invuln) == 1  # exactly one INVULN bar (no duplicate from the dodge/getup source)
    assert invuln[0].color == rb.INVULN_BAR_COLOR


def test_invuln_bar_stacks_with_the_grabs_left_dots():
    # The #720 stack: during a hang both the INVULN bar (#658) and the grabs-left dots
    # (#657) show — neither suppresses the other.
    plats = _stage()
    ledges = ledges_from_platforms(plats)
    p = _player()
    _grab_left(p, plats, ledges)
    assert any(b.label == "INVULN" for b in rb.timer_bar_specs(p))
    assert rb.grabs_left_dots(p) == 5  # first grab -> 5 dots, live alongside the bar


def test_residual_bar_normalized_per_grant():
    # A 6th-grab (past-cutoff) residual: ledge_invuln_granted == the 5f residual, so the
    ...
    _grab_left(p, plats, ledges)
    p.fighter.ledge_invuln_timer = residual
    p.fighter.ledge_invuln_granted = residual
    (bar,) = [b for b in rb.timer_bar_specs(p) if b.label == "INVULN"]
