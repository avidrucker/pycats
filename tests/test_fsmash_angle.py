...
def test_angle_helper_replaces_only_the_angle():
    boxes = (
        Hitbox(circle=Circle(78, 30, 12), damage=18.0, angle=361, base_knockback=35.0, knockback_growth=95.0),
        Hitbox(circle=Circle(52, 30, 14), damage=14.0, angle=361, base_knockback=25.0, knockback_growth=72.0),
    )
...
def test_up_angled_fsmash_spawns_with_the_up_angle():
    p, plats = _grounded()
    atk = _fire_and_capture(p, plats, _frame(held=("smash", "right", "up"), pressed=("smash",)))
...
def test_straight_fsmash_keeps_its_authored_angle():
    p, plats = _grounded()
    atk = _fire_and_capture(p, plats, _frame(held=("smash", "right"), pressed=("smash", "right")))
    assert atk is not None
