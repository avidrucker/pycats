...
try:
    ...
    def test_dodge(test_name, start_in_shield, direction, dir_key):
        ...
        player = Player(500, 400, controls, (255, 160, 64), (255, 255, 255), test_name.replace(' ', ''))
        
        # Settle on platform
        settle = InputFrame(held=set(), pressed=set(), released=set())
        player.update(settle, platforms, pg.sprite.Group())
        print(f"Settled: pos={player.rect.center}, on_ground={player.on_ground}, state={player.fsm.state}")
        ...
        print(f"After trigger: state={player.fsm.state}, vel={player.vel}, timer={player.dodge_timer}")
        
        # Run full dodge duration
        continue_frame = InputFrame(held={pg.K_q, dir_key}, pressed=set(), released=set())
    ...
    if abs(idle_diff) > 5:
        print(f"❌ IDLE ASYMMETRY: Right={results['idle_right']}, Left={results['idle_left']}, Diff={idle_diff}")
    else:
    ...
    if abs(shield_diff) > 5:
        print(f"❌ SHIELD ASYMMETRY: Right={results['shield_right']}, Left={results['shield_left']}, Diff={shield_diff}")
    else:
    ...
    if right_consistency > 5:
        print(f"❌ RIGHT INCONSISTENCY: Idle={results['idle_right']}, Shield={results['shield_right']}, Diff={right_consistency}")
    else:
        print(f"✅ Right dodge consistent between states")
        
    if left_consistency > 5:
        print(f"❌ LEFT INCONSISTENCY: Idle={abs(results['idle_left'])}, Shield={abs(results['shield_left'])}, Diff={left_consistency}")
    else:
