...
def test_controllers_drives_both_players_in_pre_contact_window():
    ...
    snaps = run_battle(frames=40, controllers=(c1, c2))
    window = 20
...
def test_single_controller_equals_controllers_tuple_with_none():
    ...
    old = run_battle(frames=200, controller=ChaseController(1))
    new = run_battle(frames=200, controllers=(ChaseController(1), None))
    assert old == new
...
def test_capture_and_replay_byte_identical():
    """Freeze the merged 2-NPC stream and confirm the replay reproduces the
    original live capture run byte-for-byte."""
    c1 = ChaseController(attacker_num=1)
    c2 = ChaseController(attacker_num=2)
    live = run_battle(frames=300, controllers=(c1, c2))
    inputs = [merge_frames([a, b]) for a, b in zip(c1.emitted, c2.emitted)]
    assert len(inputs) == len(live) == 300

    replay = run_battle(frames=len(inputs), frame_inputs=inputs)
    assert live == replay, "captured stream does not reproduce the live run"


def test_controller_and_controllers_together_is_an_error():
    c = ChaseController(1)
    with pytest.raises(ValueError):
        run_battle(frames=10, controller=c, controllers=(c, None))
