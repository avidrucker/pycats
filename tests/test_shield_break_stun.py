...
def test_render_battle_invokes_dizzy_for_stunned_player(monkeypatch):
    ...
    calls = []
    # #1315: render_battle now passes offset=(0,0) to the status helpers (throwaway
    # camera world-draw shift; identity by default), so the stub accepts it.
    monkeypatch.setattr(
        battle, "draw_dizzy_stars", lambda surf, p, offset=(0, 0): calls.append(p.fighter.stun_timer)
    )
