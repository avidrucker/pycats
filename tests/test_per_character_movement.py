...
def test_fighter_data_carries_custom_movement():
    ...
    fd = FighterData(hurtbox=_HB, moves={}, gravity=0.9, max_fall_speed=25, move_speed=10, jump_vel=-20, max_jumps=5)
    assert (fd.gravity, fd.max_fall_speed, fd.move_speed, fd.jump_vel, fd.max_jumps) == (0.9, 25, 10, -20, 5)
...
def test_fighter_threads_movement_and_weight_from_data():
    ...
    fd = FighterData(
        hurtbox=_HB, moves={}, weight=120, gravity=0.9, max_fall_speed=25, move_speed=10, jump_vel=-20, max_jumps=5
    )
    f = Fighter(x=100, y=100, facing_right=True, fighter_data=fd)  # #264/S6: no owner
    assert f.weight == 120
    assert (f.gravity, f.max_fall_speed, f.move_speed, f.jump_vel, f.max_jumps) == (0.9, 25, 10, -20, 5)
