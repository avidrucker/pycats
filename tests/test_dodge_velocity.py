...
def test_dodge_velocity():
    ...
    player = Player(
        x=400,
        y=400,  # Center of large platform
        controls={
            "left": pg.K_a,
            "right": pg.K_d,
            "up": pg.K_w,
    ...
    print(
        f"After dodge: vel={player.vel}, state={player.fsm.state}, dodge_timer={player.dodge_timer}"
    )
    ...
    for i in range(5):
        ...
        print(
            f"Frame {i + 1}: vel={player.vel}, moved={distance}px, pos={player.rect.center}"
        )
