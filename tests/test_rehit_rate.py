...
def test_movedata_rehit_rate_defaults_to_none():
    m = MoveData(name="x", in_air=False, startup=1, active=2, recovery=1, hitboxes=(_overlapping_box(),))
    assert m.rehit_rate is None


def test_movedata_accepts_rehit_rate():
    m = MoveData(
        name="drill", in_air=True, startup=1, active=20, recovery=1, hitboxes=(_overlapping_box(),), rehit_rate=4
    )
